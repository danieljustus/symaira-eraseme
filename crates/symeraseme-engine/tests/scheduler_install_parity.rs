...
fn seed_legacy(home: &Path, platform: Platform, python: bool) {
...
fn seed_status_names(home: &Path, platform: Platform) {
    let (dir, names): (PathBuf, [&str; 3]) = match platform {
        Platform::Launchd => (
            home.join("Library").join("LaunchAgents"),
            [
    ...
    fs::create_dir_all(&dir).expect("status dir");
    for name in names {
        fs::write(
            dir.join(name),
            "# Generated by symeraseme generate-scheduler (Go)\n",
...
fn rust_install_status_uninstall_match_the_go_capture() {
    ...
    for (name, case) in cases {
        ...
        if scenario.legacy {
            seed_legacy(&home, scenario.platform, true);
        }
        if scenario.seed_install_dir {
            seed_legacy(&home, scenario.platform, false);
        }
        if scenario.status_names {
            seed_status_names(&home, scenario.platform);
        }
...
impl Scenario {
    ...
    fn for_case(name: &str) -> Self {
        let (platform, op) = if name.starts_with("cron_") {
            (Platform::Cron, name)
        ...
        let op = if op.contains("status") {
            Op::Status
        } else if op.contains("uninstall") {
            Op::Uninstall
        } else {
            Op::Install
        };
        Self {
            ...
            legacy: name.contains("python_legacy"),
            replace: name.contains("replaces_python_legacy"),
            // Mirrors the oracle's `seedInstallDir` flag: reports_active,
            // removes_units and names_are_gone all pre-seed the install dir.
            seed_install_dir: name.contains("reports_active")
                || name.contains("removes_units")
                || name.contains("names_are_gone"),
            status_names: name.contains("names_differ") || name.contains("names_are_gone"),
