...
pub(super) const LEGACY_MARKER: &str = "Generated by symeraseme generate-scheduler";
...
/// Reports whether a unit file was written by this implementation, identified
/// by the Go marker. Re-running `install` must not refuse on its own output;
/// only a genuinely foreign unit requires explicit replacement consent.
fn is_own_generated_unit(content: &str) -> bool {
    content.contains(&format!("{LEGACY_MARKER} (Go)"))
}
...
pub fn scan_legacy_python_units(
    ...
    for &name in names {
        ...
        if is_own_generated_unit(&String::from_utf8_lossy(&bytes)) {
            continue;
        }
