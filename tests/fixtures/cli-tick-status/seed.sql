-- Frozen input rows for the CLI `plan status` / `plan tick` contract
-- (rust-tests/parity/oracle/cli-tick-status). One statement per line; the Go oracle and the
-- Rust replay test execute this file verbatim, so both sides read identical rows.
--
-- Why frozen rows: the Go write path stamps created_at/recorded_at/deadline_at with time.Now()
-- and the CLI offers no injection point, so a store the product wrote could never be pinned.
--
-- The campaign row is required: `plan status` counts campaigns first
-- (reporting.GetCampaignStatus -> loadCampaigns), so requests alone report `requests: 0`.
--
-- Three requests in one campaign:
--   * request 1 is SENT with an overdue deadline and a due next action,
--   * request 2 is SENT without a deadline,
--   * request 3 is CONFIRMED, so it counts as resolved.
INSERT INTO campaigns (id, created_at, kind, notes) VALUES ('campaign-1', '2026-08-06 09:00:00', 'initial', 'frozen fixture');
