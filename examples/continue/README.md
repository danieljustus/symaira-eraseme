...
[Continue](https://continue.dev) is an open-source AI coding assistant. It does **not** support SKILL.md natively but uses `.continue/rules/` and `.continuerc.json`.
...
Create `.continue/rules/symaira-eraseme.md`:
...
```json
{
  ...
  "context": {
    "symaira-eraseme": {
      "description": "GDPR/CCPA data broker removal tool",
      "commands": [
...
Continue automatically applies rules when relevant context is detected.
