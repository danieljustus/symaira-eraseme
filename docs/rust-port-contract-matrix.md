...
| CLI-018 | CLI | dashboard/calendar/requests/events JSON shapes | golden DB | read-model CLI cases | byte | all | PARTIAL (`dashboard`, `calendar` and `requests list` are replayed byte-exactly against the recorded oracle: `dashboard-json`/`operate-dashboard` (`dashboard --output json`) match `reporting::get_dashboard_data`, `calendar-json`/`operate-calendar` match `get_calendar` and `requests-list-json`/`operate-requests-list` match `list_requests`; all four commands are thin wrappers over `mcp.ContractHandler()`, so `crates/symeraseme-cli/src/cli.rs` (`contract_command`) calls the MCP handler, which gained the `get_dashboard_data` and `get_calendar` arms (`crates/symeraseme-cli/src/mcp/handler.rs`). The only folded values are the three the oracle itself folds (`generated_at`, `as_of`, `horizon_until`), and each is still asserted to parse as RFC 3339 — Go renders those with `2006-01-02T15:04:05.999999-07:00`, so a UTC instant ends in `+00:00` and not `Z`, which the masker enforces. `events show` remains a deferred stub, so the row is partial. Removing the CLI dispatch arms or the handler arms fails the replay) |
| CLI-019 | CLI | reports/dashboard files | golden DB | report CLI cases | byte+filesystem | all | TODO |
| CLI-020 | CLI | manual task list/show/complete/cleanup | golden DB/temp files | manual-task CLI cases | byte+side-effect | all | PARTIAL (`manual-tasks list` is replayed byte-exactly: `manual-tasks-list-json`/`operate-manual-tasks-list` match `{"message":"No manual tasks found.","success":true,"tasks":[]}` through `mcp.ContractHandler()`'s existing `manual_tasks_list` arm, with no folded value at all. `show`, `complete` and `cleanup` remain deferred stubs, so the row is partial) |
