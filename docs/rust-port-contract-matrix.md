...
| MCP-003 | MCP | `tools/call` request validation plus real execution for the tools whose cores exist; the rest report their state explicitly | Go `ContractHandler()`; `tests/fixtures/mcp-contract/mcp-003/cases.json` (4 measured cases) | `crates/symeraseme-cli/src/mcp/{tools_call,handler}.rs`, fixture loop | byte/semantic | all | PARTIAL (`redact_file` executes and matches Go's real handler byte-exactly, including the opaque workspace read error and the legacy-alias default; the remaining catalogue tools are wired as their cores land — `list_brokers`, `validate`, `generate_scheduler`, `grant`, `manual_tasks_*`, `plan_create`, `get_calendar`, `get_dashboard_data`) |
